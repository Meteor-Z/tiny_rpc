...
IPv4NetAddr::IPv4NetAddr(std::string_view ip, uint16_t port) : m_ip(ip), m_port(port) {
    ...
    DEBUG_LOG("IPv4NetAddr init success.");
}

IPv4NetAddr::IPv4NetAddr(std::string_view addr) {
    size_t pos = addr.find_first_of(":");
    if (pos == addr.npos) {
        ERROR_LOG(fmt::format("invalideu ipv4 addr {}", addr));
        return;
    }
    m_ip = addr.substr(0, pos);
    DEBUG_LOG(fmt::format("IPNetAddr() initing... m_ip = {}", m_ip));

    std::string temp_port { addr.substr(pos + 1) };
    m_port = std::atoi(temp_port.c_str());
    DEBUG_LOG(fmt::format("IPNetAddr() initing.... m_port = {}", m_port));
...
bool IPv4NetAddr::check_valid() {
    if (m_ip.empty()) {
        DEBUG_LOG(fmt::format("ip is empty(), error"));
        return false;
    }
    ...
    if (inet_addr(m_ip.c_str()) == INADDR_NONE) {
        DEBUG_LOG(fmt::format("can not to transform"));
        return false;
    }
