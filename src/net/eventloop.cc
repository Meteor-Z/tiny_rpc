...
EventLoop::EventLoop() {
    ...
    if (m_wakeup_fd < 0) {
        rpc::utils::ERROR_LOG(
            fmt::format("不能够创建eventfd ,error info {}", errno));
        exit(0);
    }
...
void EventLoop::init_wakeup_fd_event() {
    ...
    if (m_wakeup_fd < 0) {
        rpc::utils::ERROR_LOG(fmt::format(
            "failed to create eventloop,, eventfd create error ,error info {}",
            errno));
        exit(1);
    }
...
void EventLoop::loop() {
    while (!m_stop_flag) {
        // 缩到下面的unlock()那里就可以了
        // 如果全部锁住，因为下面的函数也用到了这个锁了，这里要改成函数式的
        ...
        int rt =
            epoll_wait(m_epoll_fd, result_event, global_epoll_max, timeout);
        rpc::utils::DEBUG_LOG(fmt::format("now end epoll_wait.. rt = {}", rt));

        if (rt < 0) {
            rpc::utils::ERROR_LOG(
                fmt::format("epoll_wait error, errno = {}", errno));
        } else {
            for (int i = 0; i < rt; i++) {
                epoll_event trigger_event = result_event[i];
                std::unique_ptr<FdEvent> fd_event_ptr =
                    std::make_unique<FdEvent>(
                ...
                if (trigger_event.events & EPOLLIN) {
                    rpc::utils::DEBUG_LOG(fmt::format(
                        "fd {} trigger EPOLLIN event", fd_event_ptr->get_fd()));
                    add_task(fd_event_ptr->handler(
                        FdEvent::TriggerEvent::IN_EVENT));
                }

                if (trigger_event.events & EPOLLOUT) {
                    rpc::utils::DEBUG_LOG(
                        fmt::format("fd {} trigger EPOLLOUT event",
                                    fd_event_ptr->get_fd()));
                    add_task(fd_event_ptr->handler(
...
void EventLoop::add_to_epoll(FdEvent* event) {
    ...
    rpc::utils::INFO_LOG(
        fmt::format("epoll_event.events = {}", (int)tmp.events));

    int rt = epoll_ctl(m_epoll_fd, op, event->get_fd(), &tmp); // 注册 添加事件
    if (rt == -1) {
        rpc::utils::ERROR_LOG(
            fmt::format("failed epoll_ctl when add fd, errno={}, error={}",
                        errno, strerror(errno)));
    }

    m_listen_fds.insert(event->get_fd());
    rpc::utils::DEBUG_LOG(
        fmt::format("add event success, fd[{}]", event->get_fd()));
}
...
void EventLoop::delete_from_epoll(FdEvent* event) {
    ...
    rpc::utils::DEBUG_LOG(
        fmt::format("delete event success, fd[{}]", event->get_fd()));
}
} // namespace rpc