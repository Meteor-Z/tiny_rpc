...
Timer::Timer() : FdEvent() {
    ...
    listen(FdEvent::TriggerEvent::IN_EVENT, std::bind(&Timer::on_time, this));
}
...
void Timer::reset_arrive_time() {
    ...
    if (rt != 0) {
        rpc::utils::ERROR_LOG(
            fmt::format("timerfd_settime error, errno = {} error = {}", errno,
                        strerror(errno)));
    }
...
void Timer::delete_time_event(TimerEvent::s_ptr event) {
    ...
    rpc::utils::DEBUG_LOG(fmt::format("success delete time_event at arrive {}",
